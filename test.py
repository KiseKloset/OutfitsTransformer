...
def main(config):
    ...
    with torch.no_grad():
        for batch_idx, (item_indices, embeddings, input_mask, target_mask) in enumerate(tqdm(data_loader)):
            item_indices = item_indices.to(device)
            embeddings = embeddings.to(device)
            input_mask = input_mask.to(device)
            target_mask = target_mask.to(device) 
            output = model(embeddings, input_mask)
            
            loss = loss_fn(output, embeddings, target_mask)
